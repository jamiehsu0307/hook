...
**`api-python-api:latest` is stale relative to `requirements.txt`** — it predates `opencc` and `pytest` being added, so a *fresh* container from that tag (not the long-running `hook_api` one, which has them installed live in its writable layer) will fail with `ModuleNotFoundError: No module named 'opencc'` on startup, or lack `pytest` entirely. If you spin up a new container from this image (e.g. to test a branch/worktree without touching the running `hook_api`), install both first: `pip install opencc pytest` before running `uvicorn`/`pytest`. Rebuilding the image from the current `Dockerfile`/`requirements.txt` would fix this properly but hasn't been done.
...
- **`POST /classify_triples`** is unrelated to the phishing generator's own guardrails — it takes a batch of 1–50 external `(subject, predicate, object)` triples (e.g. from a knowledge graph) and asks the LLM, via guided decoding, to classify each one against the same four `classification.json` axes (`scenario`/`mechanism`/`lever`/`action`). No authentication (same as every other endpoint in this app — an API key was added and then removed during development). **Known limitation, not a bug**: `scenario`/`mechanism`/`lever`/`action` are deliberately orthogonal axes (see `task.md`) describing a *phishing email's* attack properties, not a generic fact's — a triple like `("台積電", "創立年份", "1987")` has no real signal for mechanism/lever/action. The prompt reframes the task as "if you were to build a training pretext from this fact, what would you pick" (rather than "extract this attribute from the triple") and each result carries a `signal_strength: "strong"|"weak"` self-report plus a one-line `reasoning`, so low-confidence guesses can be filtered downstream — but the four-axis classification is fundamentally a best-effort guess for triples with no attack context, not a precise label. Guided-decoding array order isn't trusted structurally: each item echoes an `index`, and `main.py` re-sorts by it and 502s if the index set doesn't match `{0..n-1}` exactly.
