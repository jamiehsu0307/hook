...
**In scope（本次要做）**
- 互動式單頁網頁（點選參數 → 生成 → 預覽）。
- 透過既有後端 `api/main.py` 的 `POST /query` 代理呼叫本機 Ollama，生成**繁體中文**演練郵件（見 §3 Backend 整合契約）。
...
不要在前端組 OpenAI 相容格式或處理 CORS——一律打既有的 `POST /query`，它已內建 SSE streaming、CORS、與 Ollama 溝通的細節。

**Request**（`POST /query`，同源、相對路徑即可）：
```json
{
  "model": "<使用者於 §4 model 下拉選單選取的值>",
  "messages": [
    { "role": "system", "content": "<§6.1 system prompt>" },
    { "role": "user", "content": "<§6.2 user prompt>" }
  ],
  "stream": true,
  "format": { "...": "見 §5 的 JSON schema，需轉成 JSON Schema 格式傳入" }
}
```
- **`format` 一定要自己帶**：`main.py` 對 `format` 做的是 `setdefault`，如果不帶，會被塞入該檔既有的「情緒分析」預設 schema（`emotion`/`confidence`/`rationale`），跟本專案完全無關。
- 模型下拉選單資料來源：`GET /tags`（回傳格式與篩選邏輯見 `main.py` 的 `LLM_FAMILIES` 白名單）。
- **要打 `GET /system-prompt`**：情緒分析功能已移除，`api/system_prompt.md` 現在存放的就是 §6.1 這份釣魚演練 system prompt（逐字相同）。前端於頁面載入時 fetch 這支端點取得內容，存進 `SYSTEM_PROMPT` 變數，作為 `messages[0]` 送出——不要把 §6.1 內容另外寫死一份在 `index.html` 裡，避免兩邊維護、內容漂移不同步（見 CLAUDE.md）。
- **要打 `GET /examples`**：§6.2 `<example>` 的 5 個 few-shot 範例存放於 `api/examples.json`（純 JSON array，非文件內嵌）。前端於頁面載入時一併 fetch 這支端點存進 `EXAMPLES` 陣列；`buildUserPrompt()` 依目前選取的 `delivery_mechanism` 從中挑出**唯一匹配的一筆**帶入 prompt（不是全部塞入，見 §6.2 說明）。之後新增/調整範例只需編輯 `api/examples.json`，不必碰 `index.html`；`main.py` 若改動 `/examples` 路由本身則需重啟服務（見 CLAUDE.md）。
...
**已知後端預設值**：非 thinking 模型時，`main.py` 會補 `options.num_predict=1500`、`options.repeat_penalty=1.3`。difficulty=3、破綻較多時可能被截斷導致 JSON.parse 失敗，前端可在 request 裡自帶 `options` 覆蓋（重試時調高 num_predict，見 §4 動作的失敗分類）。
...
**動作**
- 「生成」按鈕 → 組 prompt → 呼叫 `POST /query`（見 §3.1）→ 解析 JSON → 渲染。
- 需有 loading 狀態與錯誤處理，區分三種失敗模式，不可整頁崩潰：
  1. **連線失敗**（`/query` fetch 失敗或非 2xx）→ 提示連線錯誤，可重試。
...
>
> 這份 schema 是透過 §3.1 的 `POST /query` request 的 `format` 欄位帶給 Ollama 做 guided decoding，不是前端自己事後驗證用的——務必轉成標準 JSON Schema（`type`/`properties`/`required`）格式再送出；`difficulty` 應宣告為 integer enum `[1,2,3]`，四個分類欄位應各自帶對應值域的 enum 約束（值域見 §6.2 `<enums>` 與 §7）；`red_flags` 應加 `"minItems": 1`，否則 guided decoding 在合法範圍內可以回傳空陣列 `[]`，直接違反 §2 guardrail 2。
...
<example>
下列 5 個範例覆蓋不同的 mechanism / lever / difficulty 組合，示範 §5.1 mechanism 相依欄位在各情境下如何對齊（尤其 `bec_no_payload`/`callback` 無連結、`oauth_consent` 帶 App 名稱這幾種較容易出錯的情況）。**實際維護位置是 `api/examples.json`（見 §3.1 `GET /examples`）**，下方僅為文件用途的展示副本，供理解設計意圖；實際送入單次 prompt 的只有其中 mechanism 匹配的那一筆，不是全部 5 筆一起送：
